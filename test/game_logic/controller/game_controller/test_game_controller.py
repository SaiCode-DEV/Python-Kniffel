...
class GameControllerTest(TestCase):
    ...
    def setUp(self):
        ...
        self.game_controller = GameController(
            self.mock_kniffel_controller, self.mock_game_window)

    def test_quit(self):
        self.game_controller.handle_input(key_codes.VK_LC_Q)
        self.assertTrue(
            self.mock_kniffel_controller.show_start_menu_called,
    ...
    def test_input_dice_forward(self):
        ...
        for _ in range(10):
            ...
            dice_rolled = [d.value
                           for d in self.game_controller.get_game_state().dice]
            self.assertEqual(
                len(dice),
                len(dice_rolled),
                "Dice count should not vary during roll")
            if len(set(dice).intersection(dice_rolled)) != len(dice_rolled):
                dice_changed = True
                break
        self.assertTrue(
            dice_changed,
            "dice did not roll either because of error in dice controller or input not forwarded to dice controller")

    def test_saving_and_loading_state(self):
        common.DIR_PERSISTENCE = game_controller.__file__.replace(
            "__init__.py", "") + "json-temp"
        common.FILE_GAME_STATE = path.join(
            common.DIR_PERSISTENCE, "game_state.json")
        ...
        for i in range(common.PLAYER_COUNT):
            values_expected = [p.value
                               for p in
                               state_generator.get_empty_combinations()
                               [i]]
            values_actual = [p.value
                             for p in self.game_controller.combinations[i]]
            self.assertListEqual(
                values_expected, values_actual,
                "Game-Controller did not reset game-state after new game started")

        equal_dice = {d.value for d in game_state.dice}.intersection(
            self.game_controller.dice_controller.get_dice_values())
        self.assertNotEqual(
            5, equal_dice,
            "game controller should have re-rolled dice after game start")
        self.assertEqual(
            0, self.game_controller.active_player,
            "game controller should have reset active player")
        ...
        self.assertEqual(
            ...
            "Game-Controller did not load game-state correctly")
    ...
    def test_entry(self):
        ...
        self.game_controller.card_controller.set_selected_combination(
            Combinations.TWOS.value)
        self.game_controller.card_controller.handle_input(
            key_codes.VK_NUMPAD_ENTER)

        self.assertEqual(10, self.game_controller.get_game_state(
        ).points[0][Combinations.TWOS.value].value, "should have added entry to twos with value 10")
    ...
    def test_available_bot_options(self):
        ...
        self.assertListEqual(
            ...
            "the available options were wrongly prepared for bot")
